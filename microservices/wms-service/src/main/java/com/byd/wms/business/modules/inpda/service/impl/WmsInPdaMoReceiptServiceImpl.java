...
import com.byd.wms.business.modules.inpda.dao.WmsInPdaMoReceiptDao;
 ...
 */
@Service
public class WmsInPdaMoReceiptServiceImpl implements WmsInPdaMoReceiptService {

	@Autowired
	private WmsInPdaMoReceiptDao wmsInPdaMoReceiptDao;
	@Autowired
	...
	public R getMorecCache(Map<String, Object> map) {
		try {
			//获取生产订单收货缓存信息
			List<Map<String,Object>> result = wmsInPdaMoReceiptDao.getMorecCache(map);
	...
	public R validlableinfo(Map<String, Object> params) {
		...
		//验证缓存表里面是否缓存该标签条码
		List<Map<String,Object>> cacheValidateMap= wmsInPdaMoReceiptDao.getMorecCache(params);
		...
		List<Map<String, Object>> retLabelList=wmsInPdaMoReceiptDao.getLabelList(labelMap);//获取页面即将查询的这一条标签
		...
		if(retLabelList!=null&&retLabelList.size()>0){
			...
			if(retLabelList!=null && retLabelList.size()>0) {
				for (Map<String, Object> map : retLabelList) {
					/**
					 * 校验： 1、条码与状态权限（工厂是否是该登录用户所选的工厂）【待之后加入用户登录所选工厂查询】,查询不到结果即是没有权限.
					 * 	2、条码状态为创建
					 * 	3、需要看工厂是否启用有效期WMS_C_WH，条码有效期（不能为空、不能小于生产日期、不能小于当前收货日期）
					 */
					//验证条码状态要为创建或者待进仓
					if (!"00".equals(map.get("LABEL_STATUS").toString()) && !"03".equals(map.get("LABEL_STATUS").toString())) {
						return R.error("该标签条码：" + map.get("LABEL_NO") + "的状态不为创建或者待进仓！");
					}
	...
	public R getMoDetailByBarcode(Map<String, Object> params) {
		...
		try {
			List<Map<String,Object>> result = wmsInPdaMoReceiptDao.getMoDetailListByBarcode(params);
			if(result == null || result.isEmpty()){
				return R.error("标签号 "+params.get("LABEL_NO")+"在条码信息中不存在！");
			}
			//先保存缓存扫描信息(通过工号/工厂/仓库号/业务类型/业务code,确定唯一性) 加上菜单code
			wmsInPdaMoReceiptDao.insertMoReceiptPdaCache(params);
	...
	public PageUtils getGridPoreDataPage(Map<String, Object> params) {
		...
		int count=wmsInPdaMoReceiptDao.getGridPoreDataCount(params);
		...
		params.put("END", end);
		List<Map<String,Object>> list=wmsInPdaMoReceiptDao.getGridPoreData(params);
	...
	public PageUtils getBarGridPoreDataPage(Map<String, Object> params) {
		...
		String pageSize=(params.get("pageSize")!=null && !params.get("pageSize").equals(""))?params.get("pageSize").toString():"15";
		int start = 0;int end = 0;
		int count=wmsInPdaMoReceiptDao.getBarGridPoreDataCount(params);
		if(params.get("pageSize")!=null && !params.get("pageSize").equals("")) {
			start = (Integer.valueOf(pageNo)-1) * Integer.valueOf(pageSize);
			end = Integer.valueOf(pageNo)*Integer.valueOf(pageSize);
		}else {
		...
		params.put("START", start);
		params.put("END", end);
		List<Map<String,Object>> list=wmsInPdaMoReceiptDao.getBarGridPoreData(params);
	...
	public R poReDeleteBarInfo(Map<String, Object> params) {
		try {
			...
			wmsInPdaMoReceiptDao.delPorecBarInfos(params);
			return R.ok();
		}catch (Exception e){
			return R.error(e.getMessage());
		}
	...
	List<Map<String, Object>> JSONArrayToList(Map<String, Object> params){
		...
		for(int i=0;i<jarr.size();i++){
			...
			List<Map<String, Object>> inboundItems = wmsInPdaMoReceiptDao.getInboundItemList(inboundParm);
...
//				itemMap.put("HEADER_TXT", itemMap.get("TXTRULE")==null?"":itemMap.get("TXTRULE"));
		 ...
		 * 保存收货日志表(标签日志表)
		 */
		wmsInPdaMoReceiptDao.insertWmsBarCodeLog((List<Map<String,Object>>) JSONArray.parse(params.get("ARRLIST").toString()));
		//删除PDA缓存表里面的数据,根据用户账号\工厂\仓库号\业务类型(#{BUSINESS_CODE}\#{USERNAME}\#{WERKS}\#{WH_NUMBER})
		wmsInPdaMoReceiptDao.delMorecCache(params);
...
//		result.put("INSPECTION_NO", INSPECTION_NO);//需要走质检接口
	...
	@Override
	@Transactional(rollbackFor=Exception.class)
	public R moReceiptPdaIn(Map<String, Object> params) {
		Map<String, Object> result = new HashMap<>();
		try {
			 ...
			 * 保存收货日志表(标签日志表)
			 */
			wmsInPdaMoReceiptDao.insertWmsBarCodeLog((List<Map<String, Object>>) params.get("matList"));
...
//			result.put("INSPECTION_NO", INSPECTION_NO);//需要走质检接口
	...
	public void saveOrUpdateCoreLabel(List<Map<String, Object>> skList, List<Map<String, Object>> matList) {
		...
		if(skList.size()>0) {
			wmsInPdaMoReceiptDao.saveOrUpdateCoreLabel(skList);
		}
	...
	public List<Map<String, Object>> getAllLabelInfos(Map<String, Object> map) {
		List<Map<String, Object>> allLabelInfos= wmsInPdaMoReceiptDao.getAllLabelInfos(map);
