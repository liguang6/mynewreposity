...
$(function () {
    ...
    $('#btnPoReBarDeleteBar').on('click', function () {
        ...
        console.log(rows);
        $.ajax({
            url: baseUrl + "returngoods/wmsReMaterialTwoWarehouse/deleteLabel",
            dataType: "json",
            type: "post",
            data: {
    ...
    $('#btnConfirmInfo').on('click', function () {
        //生成抬头文本
        $.ajax({
            url: baseUrl + "returngoods/wmsReMaterialTwoWarehouse/createheadText",
            dataType: "json",
            type: "post",
            data: {
                "PZ_DATE": $("#docDate").val(),
    ...
    $('#btnConfirm').on('click', function () {
        ...
        $.ajax({
            url: baseUrl + "returngoods/wmsReMaterialTwoWarehouse/poReceiptPdaIn",
            dataType: "json",
            type: "post",
            data: {
                "PZ_DATE": $("#docDate").val(),
    ...
    //初始化明细数据grid
    $("#poreDataGrid").jqGrid({
        url : baseUrl + "returngoods/wmsReMaterialTwoWarehouse/getGridPoreData",
        datatype: "json",
        jsonReader: jsonreader,
        colNames: ['物料号', '<span style="color:red">*</span><b>生产订单</b></span>', '<a style="color:red">*</a>接收库位','<a style="color:red">*</a>托盘号', '总数量', '箱数', '库存类型', '供应商代码','批次',
            '','','',''],
        colModel: [
            {name: 'MATNR', index: 'MATNR', width: 80, align: 'center'},
            {name:'MO_NO',index:'MO_NO', width:53, align:'center',editable:true},
...
function initPoCacheInfo() {
    $.ajax({
        url: baseUrl + "returngoods/wmsReMaterialTwoWarehouse/getPorecCache",
        dataType: "json",
        type: "post",
...
function viewPoBarInfo(rowId) {
    ...
    $("#poreBarDataGrid").jqGrid('setGridParam',{
        url:baseUrl + "returngoods/wmsReMaterialTwoWarehouse/getBarGridPoreData",
        datatype : 'json',
        postData : rowData,
        page : 1
...
function poreScanPre() {
    $.ajax({
        url: baseUrl + "returngoods/wmsReMaterialTwoWarehouse/validatePoReceiptLable",
        dataType: "json",
        type: "post",
        ...
        success: function (resp1) {
            if (resp1.code == '0') {
                $.ajax({
                    url: baseUrl + "returngoods/wmsReMaterialTwoWarehouse/getPoDetailByBarcode",
                    dataType: "json",
                    type: "post",
                    data: {"LABEL_NO": $("#barcode").val(),
                        "TPNUM":$("#TPNUM").val()},
