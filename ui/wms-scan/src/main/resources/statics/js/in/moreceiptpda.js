...
$(function () {
    ...
    $('#btnPoReBarDeleteBar').on('click', function () {
        ...
        // js.loading();
        $.ajax({
            url: baseUrl + "inpda/wmsmoreceiptpda/poReDeleteBarInfo",
            dataType: "json",
            type: "post",
            data: {
    ...
    $('#btnConfirmInfo').on('click', function () {
        //生成抬头文本
        $.ajax({
            url: baseUrl + "inpda/wmsmoreceiptpda/createheadText",
            dataType: "json",
            type: "post",
            data: {
                "PZ_DATE": $("#docDate").val(),
...
//         obj.setSelection(rowid);
        ...
        $.ajax({
            url: baseUrl + "inpda/wmsmoreceiptpda/moReceiptPdaIn",
            dataType: "json",
            type: "post",
            data: {
                "PZ_DATE": $("#docDate").val(),
    ...
    //初始化明细数据grid
    $("#poreDataGrid").jqGrid({
        url : baseUrl + "inpda/wmsmoreceiptpda/getGridPoreData",
        datatype: "json",
        jsonReader: jsonreader,
        colNames: ['行项', '订单号', '物料号', '总数量', '箱数', '工厂', '库位',
            'MAKTX', 'MENGE', 'MAX_MENGE', 'UNIT', 'FULL_BOX_QTY', 'PRODUCT_DATE',
            'LIKTX', 'LIFNR', 'BEDNR', 'AFNAM', 'SOBKZ','CA_LABEL_NO'],
        colModel: [
            {name: 'INBOUND_ITEM_NO', index: 'INBOUND_ITEM_NO', width: 50, align: 'center'},
            {name: 'INBOUND_NO', index: 'INBOUND_NO', width: 80, align: 'center'},
            {name: 'MATNR', index: 'MATNR', width: 80, align: 'center'},
            {name: 'MAY_IN_QTY', index: 'MAY_IN_QTY', width: 40, align: 'center'},
...
function initPoCacheInfo() {
    $.ajax({
        url: baseUrl + "inpda/wmsmoreceiptpda/getMorecCache",
        dataType: "json",
        type: "post",
...
function viewPoBarInfo(rowId) {
    ...
    $("#poreBarDataGrid").jqGrid('setGridParam',{
        url:baseUrl + "inpda/wmsmoreceiptpda/getBarGridPoreData",
        datatype : 'json',
        postData : rowData,
        page : 1
...
function poreScanPre() {
    ...
    $.ajax({
        url: baseUrl + "inpda/wmsmoreceiptpda/validatePoReceiptLable",
        dataType: "json",
        type: "post",
        data: {
        ...
        success: function (resp1) {
            if (resp1.code == '0') {
                $.ajax({
                    url: baseUrl + "inpda/wmsmoreceiptpda/getMoDetailByBarcode",
                    dataType: "json",
                    type: "post",
                    data: {"LABEL_NO": $("#porebarcode").val()},
                    success: function (resp2) {
