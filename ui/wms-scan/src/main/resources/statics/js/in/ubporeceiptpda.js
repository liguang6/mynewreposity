...
$(function () {
    ...
    $('#btnPoReBarDeleteBar').on('click', function () {
        ...
        // js.loading();
        $.ajax({
            url: baseUrl + "inpda/wmsubporeceiptpda/poReDeleteBarInfo",
            dataType: "json",
            type: "post",
            data: {
            ...
            success: function (resp) {
                if (resp.code == '0') {
                    ...
                    alertMsg("divSysInfo","删除条码信息成功! ",3000);
                    // js.closeLoading();
                } else {
                    $("#poreBarDataGrid").jqGrid('clearGridData');
                    // $("#btnsaveinteralbound").html("执行");
                    alertMsg("divSysError2",""+resp.msg,3000);
                    // js.closeLoading();
                }
            }
        });
    ...
    $('#btnConfirmInfo').on('click', function () {
        //生成抬头文本
        $.ajax({
            url: baseUrl + "inpda/wmsubporeceiptpda/createheadText",
            dataType: "json",
            type: "post",
            data: {
                "PZ_DATE": $("#docDate").val(),
            ...
            success: function (resp2) {
                if (resp2.code == '0') {
                    ...
                    alertMsg("divSysError2",""+resp2.msg,3000);
                }
            }
        })
    });
...
//         obj.setSelection(rowid);
        ...
        $.ajax({
            url: baseUrl + "inpda/wmsubporeceiptpda/ubPOReceiptPdaIn",
            dataType: "json",
            type: "post",
            data: {
                "PZ_DATE": $("#docDate").val(),
            ...
            success: function (resp2) {
                if (resp2.code == '0') {
                    ...
                    $("#pore_inbound_no").val(resp2.result.INBOUND_NO);
                    ...
                    alertMsg("divSysError2",""+resp2.msg,30000);
                }
            }
        })
    });
    ...
    //初始化明细数据grid
    $("#poreDataGrid").jqGrid({
        url : baseUrl + "inpda/wmsubporeceiptpda/getGridPoreData",
        datatype: "json",
        jsonReader: jsonreader,
        colNames: ['行项', '需求号', '物料号', '总数量', '箱数', '工厂', '库位','需求总数量','采购订单号','采购订单行号',
            'MAKTX', 'MENGE', 'MAX_MENGE', 'UNIT', 'FULL_BOX_QTY', 'PRODUCT_DATE',
            'LIKTX', 'LIFNR', 'BEDNR', 'AFNAM', 'SOBKZ','CA_LABEL_NO'],
        colModel: [
            {name: 'REQUIREMENT_ITEM_NO', index: 'REQUIREMENT_ITEM_NO', width: 50, align: 'center'},
            {name: 'REQUIREMENT_NO', index: 'REQUIREMENT_NO', width: 100, align: 'center'},
            ...
            {name: 'WERKS', index: 'WERKS', width: 30, align: 'center'},
            {name: 'LGORT', index: 'LGORT', width: 30, align: 'center'},
            {name: 'REQITEM_QTY', index: 'REQITEM_QTY', width: 80, align: 'center'},
            {name: 'PO_NO', index: 'PO_NO', width: 80, align: 'center'},
            {name: 'PO_ITEM_NO', index: 'PO_ITEM_NO', width: 100, align: 'center'},
    ...
    //初始化需求单明细数据grid
    $("#ubReqitemDataGrid").jqGrid({
        url : baseUrl + "inpda/wmsubporeceiptpda/getGridReqItemData",
        datatype: "json",
        jsonReader: jsonreader,
        colNames: ['行项', '需求号', '物料号', '数量', '库位','采购订单号','采购订单行号',
            'MAKTX', 'MENGE', 'MAX_MENGE', 'UNIT', 'FULL_BOX_QTY', 'PRODUCT_DATE',
            'LIKTX', 'LIFNR', 'BEDNR', 'AFNAM', 'SOBKZ','CA_LABEL_NO'],
        colModel: [
            {name: 'REQUIREMENT_ITEM_NO', index: 'REQUIREMENT_ITEM_NO', width: 50, align: 'center'},
            {name: 'REQUIREMENT_NO', index: 'REQUIREMENT_NO', width: 80, align: 'center'},
            {name: 'MATNR', index: 'MATNR', width: 80, align: 'center'},
            {name: 'QTY', index: 'QTY', width: 40, align: 'center'},
            {name: 'LGORT', index: 'LGORT', width: 30, align: 'center'},
            {name: 'PO_NO', index: 'PO_NO', width: 30, align: 'center'},
            {name: 'PO_ITEM_NO', index: 'PO_ITEM_NO', width: 30, align: 'center'},
...
function initPoCacheInfo() {
    $.ajax({
        url: baseUrl + "inpda/wmsubporeceiptpda/getMorecCache",
        dataType: "json",
        type: "post",
        ...
        success: function (resp) {
            if (resp.code == '0') {
                if (resp.result.length > 0) {
                    ...
                    $("#requirementNO").val(resp.result[0].REQUIREMENT_NO);

                    $("#pore_labelid").val(resp.result[0].LABEL_NO);
 ...
 * 查询需求单明细信息
 */
function getReqItemsInfo() {
    // var rowData = $("#poreDataGrid").jqGrid('getRowData',rowId);
    // console.log(rowData);
    // showBarDataGrid();
    $("#ubReqitemDataGrid").jqGrid('setGridParam',{
        url:baseUrl + "inpda/wmsubporeceiptpda/getGridReqItemData",
        datatype : 'json',
        postData :{"REQUIREMENT_NO": $("#requirementNO").val()},
        page : 1
    },true).trigger("reloadGrid");
    document.getElementById("div_reqitemdata_info").style.display = "";
...
function viewPoBarInfo(rowId) {
    ...
    $("#poreBarDataGrid").jqGrid('setGridParam',{
        url:baseUrl + "inpda/wmsubporeceiptpda/getBarGridPoreData",
        datatype : 'json',
        postData : rowData,
        page : 1
    },true).trigger("reloadGrid");
...
function changePoReTab(tabFlag) {
    if (tabFlag == "input") {
        ...
        document.getElementById("btnConfirmInfo").style.display = "none";
        document.getElementById("btnConfirm").style.display = "none";
    }else if (tabFlag == "reqitem") {
        document.getElementById("div_input").style.display = "none";
        document.getElementById("div_datainfo").style.display = "none";//隐藏
        document.getElementById("div_headinfo").style.display = "none";//隐藏
        document.getElementById("div_headconfirm_result").style.display = "none";
        document.getElementById("btnPoReDeleteBar").style.display = "none";//隐藏
        document.getElementById("btnConfirmInfo").style.display = "none";
        document.getElementById("btnConfirm").style.display = "none";
        document.getElementById("div_reqitemdata_info").style.display = "";
...
function poreScanPre() {
    var allRowIds = $('#poreDataGrid').getDataIDs();
    // if (allRowIds.length == 0) {
    // alertMsg("divSysError2","收货数据为空",3000);
    //     return;
    // }
    var rows = [];
    for (var i = 0; i < allRowIds.length; i++) {
    ...
    $.ajax({
        url: baseUrl + "inpda/wmsubporeceiptpda/validatePoReceiptLable",
        dataType: "json",
        type: "post",
        data: {
            "LABEL_NO": $("#porebarcode").val(),
            "REQUIREMENT_NO": $("#requirementNO").val(),
            "ARRLIST":JSON.stringify(rows)
        },
        success: function (resp1) {
            if (resp1.code == '0') {
                $.ajax({
                    url: baseUrl + "inpda/wmsubporeceiptpda/getMoDetailByBarcode",
                    dataType: "json",
                    type: "post",
                    data: {
                        "LABEL_NO": $("#porebarcode").val(),
                    ...
                    success: function (resp2) {
                        if (resp2.code == '0') {
                            ...
                            $("#pore_labelid").val(resp2.result.LABEL_NO);
                            $("#pore_boxqty").val(hiboxqty);
                ...
                alertMsg("divSysError2",""+resp1.msg,3000);
            }
        }
    })
    // alertMsg($("#porebarcode").val());
