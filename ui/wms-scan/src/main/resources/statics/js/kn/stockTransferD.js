	  ...
	  $("#btn5").click(function(){
		  ...
		  if (ids != "") {
			  document.getElementById("divInfo").style.visibility="hidden";
				$.ajax({
					type: "POST",
					url : baseUrl + "kn/stockTransferD/deleteLabel",
				    async:false,
					dataType:"json",
					data:{
...
function showGrid(){
	...
	$("#dataGrid").jqGrid({
    	url : baseUrl + "kn/stockTransferD/list",
    	datatype: "json",
...
function editFunction(rowID){	
	...
	$("#dataGridA").jqGrid('setGridParam',{
		url:baseUrl + "kn/stockTransferD/labelList",
		datatype : 'json',
		postData : rowData,
		page : 1
	},true).trigger("reloadGrid");
...
function save(){
	...
	$.ajax({
		url:baseUrl+'kn/stockTransferD/save',
		type:"post",
		async:true,
		dataType:"json",
		data:{
...
function getLgortList(){
	var list=[];
	$.ajax({
		url:baseUrl+'kn/stockTransferD/getLoList',
		type:'post',
		async:false,
		dataType:'json',
		success:function(resp){
