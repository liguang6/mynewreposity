	  ...
	  $("#btn5").click(function(){
		  ...
		  if (ids != "") {
			  document.getElementById("divInfo").style.visibility="hidden";
				$.ajax({
					type: "POST",
					url : baseUrl + "kn/stockTransferA/deleteLabel",
				    async:false,
					dataType:"json",
					data:{
...
function showGrid(){
	$("#dataGrid").jqGrid({
    	url : baseUrl + "kn/stockTransferA/list",
    	datatype: "json",
...
function editFunction(rowID){	
	...
	$("#dataGridA").jqGrid('setGridParam',{
		url:baseUrl + "kn/stockTransferA/labelList",
		datatype : 'json',
		postData : rowData,
		page : 1
	},true).trigger("reloadGrid");
...
function save(){
	...
	$.ajax({
		url:baseUrl+'kn/stockTransferA/save',
		type:"post",
		async:true,
		dataType:"json",
		data:{
...
function getLgortList(){
	var list=[];
	$.ajax({
		url:baseUrl+'kn/stockTransferA/getLoList',
		type:'post',
		async:false,
		dataType:'json',
		success:function(resp){
