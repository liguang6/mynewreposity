	  ...
	  $("#btn5").click(function(){
		  ...
		  if (ids != "") {
			  document.getElementById("divInfo").style.visibility="hidden";
				$.ajax({
					type: "POST",
					url : baseUrl + "kn/stockTransferB/deleteLabel",
				    async:false,
					dataType:"json",
					data:{
...
function showGrid(){
	$("#dataGrid").jqGrid({
    	url : baseUrl + "kn/stockTransferB/list",
    	datatype: "json",
    	jsonReader: jsonreader,
    	colNames:['物料号','<span style="color:red">*</span><span style="color:blue">供应商代码</span>','<span style="color:red">*</span><span style="color:blue">库位</span>','数量','箱数','源库位','批次','条码','单位','源供应商'],
	    colModel:[
	    {name:'MATNR',index:'MATNR', width:80, align:'center',sortable:false},
	    {name:'F_LIFNR',index:'F_LIFNR', width:80, align:'center',sortable:false,
	    ...
	    {name:'LIFNR',index:'LIFNR', width:80, align:'center',sortable:false,hidden:true}
	    ],
	    loadonce: true,
	    cellEdit: true,
...
function editFunction(rowID){	
	...
	$("#dataGridA").jqGrid('setGridParam',{
		url:baseUrl + "kn/stockTransferB/labelList",
		datatype : 'json',
		postData : rowData,
		page : 1
	},true).trigger("reloadGrid");
...
function saveA(){
	...
	for(var i =0; i<ids.length; i++) {
		...
		if(row.F_LIFNR==null||row.F_LIFNR==""){
			alert(ids[i]+"供应商代码不能为空");
			return false;
		}
...
function save(){
	...
	$.ajax({
		url:baseUrl+'kn/stockTransferB/save',
		type:"post",
		async:true,
		dataType:"json",
		data:{
...
function getLgortList(){
	var list=[];
	$.ajax({
		url:baseUrl+'kn/stockTransferB/getLoList',
		type:'post',
		async:false,
		dataType:'json',
		success:function(resp){
