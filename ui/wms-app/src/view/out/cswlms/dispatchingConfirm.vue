...
<template>
  <v-ons-page>
    <custom-toolbar :title="'总装拣配确认'" :action="toggleMenu"></custom-toolbar>
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.setPage('dispatching_confirm')
    ...
    methods: {
      ...mapActions([
        'DispatchingConfirmList','DispatchingConfirm','DispatchingConfirmUpdata'
      ]),
      ...mapMutations([
        'setPage','setPageDataList','setCheckDataList'
      ]),
      ScanBarcode(e){
      ...
      search(){
          if('' === this.barCode){
            ...
            if(check){
              ...
              this.DispatchingConfirmList({barCode:barCode,fromPlantCode:this.st_userWerks}).then(res => {
                if(0 === res.data.code){
      ...
      confirm(){
        console.log('-->pagedata_barcode: ' ,this.pageData.list.length)
        console.log('-->checkDataList: ' ,this.checkDataList.length)
        let confirmList = []
        for(var i = 0;i<this.checkDataList.length;i++){
          confirmList[i] = JSON.stringify(this.pageData.list[this.checkDataList[i]])
        }
      console.log('-->checkdata: ' ,confirmList)
      if(this.checkDataList.length>0){
        this.DispatchingConfirm({list:confirmList}).then(res=>{
          if(0 === res.data.code){
            this.list=[]
            this.DispatchingConfirmUpdata({list:confirmList}).then(res=>{
              if(0 === res.data.code){
                this.pageData.list=[]
                this.$ons.notification.toast('拣配确认成功', {timeout: 2000});
              }
            }).catch(e=>{
