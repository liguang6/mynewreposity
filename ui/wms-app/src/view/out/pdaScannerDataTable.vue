...
<script>
    ...
    export default {
        ...
        this.createDate = this.tools.getFormatDate(new Date(), "YYYY-MM-DD HH:MM:SS");
        },
        created(){
                var listData = [];
                var temp = [] ;
        ...
        methods : {
            ...
            create(){
                if(this.st_pageDataList[0]&&!this.st_pageDataList[0].BUSINESS_NAME){
                    this.$ons.notification.toast('请选择业务类型',{timeout:1000})
                    return
                }
                let t = this.st_pageDataList[0].BUSINESS_NAME
                if(this.$refs.mytable.selected().length === 0){
                    ...
                    this.dataTable = this.st_pageDataList.filter((v,i)=>{
                        ...
                        "BOX_SN":v.BOX_SN,"BOX_QTY":v.BOX_QTY,"UNIT":v.UNIT,postVehicleID:v.postVehicleID,CREATOR:sessionStorage.getItem('UserName'),CREATE_DATE:this.createDate,"BATCH":v.BATCH};
                        tagList= dataNew;
            ...
            back(){
                this.$store.commit("setPage",'pdaScannerDataTable');
                this.$emit("gotoPageEvent",'pdaScanner');
            },
            del(){
