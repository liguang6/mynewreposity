...
<template>
  <v-ons-page>
    <custom-toolbar :title="'库存冻结'" :action="toggleMenu"></custom-toolbar>
    <v-ons-card>
      <v-ons-row>
        <v-ons-col width="45%" ><center><v-ons-input id="assno" autocomplete="off" @keypress="ScanLabelno" autofocus=“true” name="标签号" :class="{'input': true, 'is-danger': errors.has('标签号') }" v-model="labelNo" width="95%" modifier="underbar" placeholder="标签号" float v-validate="'required'"></v-ons-input></center></v-ons-col>
    ...
    <v-ons-card>
      ...
      <v-ons-list>
        ...
        <v-ons-row v-for="(li, $index) in pageData.list" :key="li.LABEL_NO">
          <v-ons-col width="4%" style="padding-top:15px">
                ...
                <v-ons-row>
                 <v-ons-col width="50%" style="padding-top:6px" >{{li.LABEL_NO}}</v-ons-col>
                  <v-ons-col width="38%" style="padding-top:6px" >{{li.MATNR}}</v-ons-col>
                   <v-ons-col width="8%" style="padding-top:6px;text-align:right;color:red" >{{li.BOX_QTY}}</v-ons-col>
                </v-ons-row>
                <div class="expandable-content">工厂/仓库号:{{li.WERKS}}/{{li.WH_NUMBER}}<br/>物料描述:{{li.MAKTX}} <br/>
                批次:{{li.BATCH}}<br/>库位:{{li.LGORT}};库存类型:{{li.SOBKZ}}<br/>
    ...
    <ons-bottom-toolbar>
      ...
      <center>
        ...
        <v-ons-button @click="confirm" style="margin: 2px 0"><v-ons-icon icon="fa-qrcode"></v-ons-icon>&nbsp;冻结</v-ons-button> &nbsp;&nbsp;
      ...
      search(){
        ...
        console.log('-->result: '  + result)
          if (!result) {
            ...
            console.log('-->search start: '  + this.labelNo)
              if(0 === res.data.code){
                if(res.data.labelData.LABEL_STATUS=='08'){
                   this.pageData.list.push( res.data.labelData);
                }else{
                   this.$ons.notification.toast('标签状态不是已上架,不能冻结', {timeout: 2000});
                }
                 this.labelNo=''
                this.msg = ''
              }else{
        ...
        confirm(){
          if(0===this.checkDataList.length){
            this.$ons.notification.toast('请选择需要冻结的数据', {timeout: 2000});
          }else{
            ...
            saveData[i] =this.pageData.list[indexArr[i]]
             ...
             this.freeze({checkDataList:saveData,type:'00'}).then(res => {
