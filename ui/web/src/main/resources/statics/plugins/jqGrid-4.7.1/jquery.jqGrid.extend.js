(function($) {
    var DataGrid = function(options, $this) {
        var dataGrid = typeof $this != "undefined" ? $this :options.dataGrid ? options.dataGrid :$("#dataGrid"), dataGridId = dataGrid.attr("id"), dataGridPage = options.dataGridPage ? options.dataGridPage :$("#" + dataGridId + "Page"), searchForm = options.searchForm ? options.searchForm :$("#searchForm"), options = $.extend({
            ...
            editurl:"clientArray",
            rowNum:-1,
            rowList : options.rowList||[15,30,50,100],
            rownumWidth:30,
            altRows:true,
            columnModel:[],
            colNames:[],
            ...
            loadComplete:function(data) {
                if (dataGridPage.length >= 1 && data) {
                    ...
                    dataGridPage.html(pageHtml(data.page,options));
                ...
                if (typeof options.dataGridLoadComplete == "function") {
                    options.dataGridLoadComplete(data);
                }
        ...
        if (options.inputPageNo.length == 0&&options.rowNum!=-1) {
            var pageNo = searchForm.data("pageNo")||1;
            searchForm.append('<input id="pageNo" name="pageNo" type="hidden" value="' + (pageNo ? pageNo :"") + '"/>');
            options.inputPageNo = $("#pageNo", searchForm);
        }
        if (options.inputPageSize.length == 0&&options.rowNum!=-1) {
        	var rowNum =options.rowNum||15;
            var pageSize = searchForm.data("pageSize")||rowNum;
        ...
        /**
         * added by xjw 2018-5-28 公用刷新按钮
         */
        if (options.btnRefresh.length > 0) {
            options.btnRefresh.unbind("click").click(function() {
                var btnRefresh = $(this);
                searchForm.submit();
                return false;
            });
        ...
        function setSelectRow(id, isCancel) {
        	console.info("isCancel:"+isCancel)
        ...
        function mergeCell(cellNames) {
            ...
            $.each(cellNames.split(","), function(idx, name) {
                var bg = trs.eq(0).children("[aria-describedby='" + dataGridId + "_" + name + "']"), index = bg.index(), rowsp = 1;
...
function pageHtml(c,options) {
    ...
    if(options.rowList){
    	var pageSelectStrFlag = false;
    	var pageSelectStr = '<li><a style="border: 0px;padding: 0px;" href="javascript:">每页显示：<select style="height: 25px;" onchange="' + c.funcName + "(" + c.first + ",$(this).val(),'" + c.funcParam + '\');">';
    	options.rowList.forEach(function(value,i){
    		if(c.pageSize==value){
    			pageSelectStrFlag = true;
    			pageSelectStr +='<option selected value='+value+' >'+value+'</option>';
    		}else{
    			pageSelectStr +='<option value='+value+' >'+value+'</option>';
    		}
    	});
    	if(pageSelectStrFlag){
    		pageSelectStr +='<option value="10000000">All</option></select></a></li>\n';
    	}else{
    		pageSelectStr +='<option selected value="10000000">All</option></select></a></li>\n';
    	}
    	
    	b.push('</ul>\n<ul class="pagination">\n');
    	b.push(pageSelectStr);
    }
...
/**
 * 合格单元格
 * @author thw 2018-06-11
 * @param gridName 表格ID
 * @param CellNames 需要合并的列名-数组
 * @returns
 */
function Merger(gridName, CellNames) {
    ...
    for (var i = 0; i < length; i += rowSpanTaxCount) {    
        ...
        for (j = i + 1; j <= length; j++) {    
            ...
            var sameFlag = true;
            for(var s= 0;s<CellNames.length;s++){  
                if(before[CellNames[s]] != end[CellNames[s]]){
                	sameFlag = false;
                }
            } 
            if (sameFlag) {    
                rowSpanTaxCount++;
                for(var index in CellNames){  
                	$("#" + gridName + "").setCell(mya[j], CellNames[index], '', { display: 'none' });   
                }
        ...
        for(var index in CellNames){  
        	$("#" + CellNames[index] + "" + mya[i] + "").attr("rowspan", rowSpanTaxCount); 
        }
