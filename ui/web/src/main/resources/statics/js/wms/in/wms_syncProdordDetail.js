...
$(function(){
	...
	$("#dataGrid").dataGrid({
		...
		data: mydata,
        colModel: [			
            {label: '生产订单批量同步', name: 'orderNo',index:"orderNo", width: "100",align:"center",sortable:false,
            	formatter:function(val){
            		return "<input type='text' style='width:98%'/>";
            	}
            },
			/*{label: '工厂代码', name: 'id',index:"id", width: "100",align:"center",
            	formatter:function(val){
            		return "<input type='text' style='width:98%'/>";
            	}
			},*/
			{label: '', name: '',index:"werks", width: "500",align:"center",sortable:false,
            	formatter:function(val){
            		return "";
            	}
			}
	...
	$("#newOperation").click(function () {		
		...
		'<td role="gridcell" style="text-align:center;" title="" aria-describedby="dataGrid_sys"><input type="text" style="width:98%">'+
		'</td><td role="gridcell" style="text-align:center;" title="" aria-describedby="dataGrid_werks">'+
	...
	$("#btnSync").click(function () {		
		...
		if("" == orderNo){
			alert("请输入或勾选需同步的生产订单！");
			return false;
		}
		$("#btnSync").val("同步数据中,请稍候...");	
		$("#btnSync").attr("disabled","disabled");
		$.ajax({
			url:baseURL+"in/wmsinreceipt/syncProdordDetail",
			dataType : "json",
			type : "post",
			data : {
				"orderNo":orderNo
