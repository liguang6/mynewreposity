...
var vm = new Vue({
	...
	methods: {
        ...
        add: function(){
        	js.addTabPage(null, '<i class="fa fa-plus" style="font-size:12px"> 增加数据字典</i>', baseURL + 'wms/config/dict_addform.html', true, true)
        },
//		
//		getInfo: function(id){
//			$.get(baseURL + "wms/dict/info/"+id, function(r){
//                vm.dict = r.dict;
//            });
//		},
...
function fun_edit(id){
	js.addTabPage(null, '<i class="fa fa-plus" style="font-size:12px"> 编辑数据字典</i>', baseURL + 'wms/config/dict_addform.html?id='+id, true, true)
}

function fun_del(id){
	console.log("-->fun_del " + id);
	confirm('确定要删除选中的记录？', function(){
		$.ajax({
			type: "POST",
		    url: baseURL + "wms/dict/deleteById?id="+id,
