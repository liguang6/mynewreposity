...
$("#uploadForm").validate({
	  submitHandler:function(e){
		  ...
		  var form = new FormData(document.getElementById("uploadForm"));
			$.ajax({
				url:baseUrl + "config/matusing/preview",
				data:form,
				type:"post",
				contentType:false,
				processData:false,
				success:function(data){
					if(data.code == 0){
						...
						$("#dataGrid").jqGrid({
						 	datatype: 'local', 
							colNames:['','工厂代码','物料号','物料描述','上线标示','有效开始日期','有效结束日期','备注','校验提示信息'],
							colModel:[
						          ...
						          {name:"useFlag",index:'useFlag',align:'center',width:100},
...
function saveUpload(){
	 ...
	 $.ajax({
		 url:baseUrl + "config/matusing/import",
		 type:"post",
		 dataType:"json",
