$(function () {
    $("#dataGrid").dataGrid({
        ...
        columnModel: [			
			...
			{ label: '是否是上线物料', name: 'useFlag', index: 'useFlag', width: 80,align:'center',formatter: function(item, index){
			...
			{ label: 'useFlag', name: 'useFlag', index: 'useFlag', hidden:true },
		],
		rowNum : 15,
  ...
  //------删除操作---------
	  $("#deleteOperation").click(function(){
		  ...
		  if (gr != null  && gr!==undefined) {
			  layer.confirm('确定删除选中的记录?', {icon: 3, title:'提示'}, function(index){
				  ...
				  $.ajax({
					type: "POST",
				    url: baseURL + "config/matusing/delById?id="+grData.id,
	  ...
	  $("#newOperation").click(function(){
		   var options = {
	    		...
	    		maxmin: true,
	    		shadeClose: true,
	    		title: '<i aria-hidden="true" class="fa fa-plus">&nbsp;</i>新增上线物料',
	    		area: ["500px", "360px"],
	    		content: baseURL+"wms/config/wms_c_mat_using_edit.html",  
	  ...
	  $("#editOperation").click(function(){
		 ...
		 if(gr !== null && gr!==undefined){
			 ...
			 var options = {
				...
				maxmin: true,
				shadeClose: true,
				title: '<i aria-hidden="true" class="fa fa-pencil-square-o">&nbsp;</i>修改上线物料',
				area: ["500px", "360px"],
				content: baseURL+'wms/config/wms_c_mat_using_edit.html',
				btn: ['<i class="fa fa-check"></i> 确定'],
				success:function(layero, index){
	...
	//-------导入操作--------
	  $("#importOperation").click(function(){
		  var options = {
				...
				maxmin: true,
				shadeClose: true,
				title: "<i class='fa fa-upload' aria-hidden='true'>&nbsp;</i> 导入上线物料",
				area: ["880px", "520px"],
				content: baseUrl + "wms/config/wms_c_mat_using_upload.html",
...
function export2Excel(){
	var column=[
      	...
      	{"title":"上线标示","class":"center","data":"useFlag"},
	...
	params=l;
	$.ajax({
		url:baseURL+"config/matusing/list",
		dataType : "json",
		type : "post",
		data : params,
		async: false,
		success: function (response) { 
		    ...
		    for(var i=0;i<results.length;i++){
		    	var useFlag=results[i].useFlag;
		    	if(useFlag=='X'){results[i].useFlag='是';}
			    if(useFlag=='0'){results[i].useFlag='否';}
		    }
		    
		}
	})
	var rowGroups=[];
	getMergeTable(results,column,rowGroups,"上线物料");	
}
