...
var vm = new Vue({
		...
		werks : {
		  handler:function(newVal,oldVal){
				...
				$.ajax({
					...
					data : {
						"WERKS":newVal,
						"MENU_KEY":'ZZJMES_BATCH_OUTPUT_REACH_REPORT',
					},
					async: true,
	...
	methods: {
		...
		query:function(){
			...
			$("#searchForm").attr("action",baseUrl + "zzjmes/report/batchOutputReachReport");
			$('#dataGrid').dataGrid({
				...
				columnModel: [
		            ...
		            {label: '检验状态', name: 'test_status',index:"test_status", width: "70",align:"center",},
		            {label: '判定-生产', name: 'product_test_result',index:"product_test_result", width: "70",align:"center",},
		            {label: '判定-品质', name: 'test_result',index:"test_result", width: "70",align:"center",},		            
		...
		exp:function(){
			...
			$("#exportForm").attr("action",baseUrl + "zzjmes/report/batchOutputReachReportExport");
...
$(function () {
	...
	var last_process = getUrlKey("last_process");
	if(undefined != last_process && 'undefined' != last_process && null != last_process && last_process!=''){
		$("#last_process").click();
	}
