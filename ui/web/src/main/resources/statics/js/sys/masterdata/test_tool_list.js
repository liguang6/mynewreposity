$(function () {
    $("#dataGrid").dataGrid({
        ...
        columnModel: [
			...
			{ label: '检具编码', name: 'TEST_TOOL_NO', index: 'TEST_TOOL_NO', width: 100,align:'center'  },
			{ label: '检具名称', name: 'TEST_TOOL_NAME', index: 'TEST_TOOL_NAME', width: 100,align:'center'  },
			{ label: '规格', name: 'SPECIFICATION', index: 'SPECIFICATION', width: 100,align:'center'  },
  ...
  //------删除操作---------
	  $("#deleteOperation").click(function(){
		  ...
		  if (gr != null  && gr!==undefined) {
			  layer.confirm('确定删除选中的记录?', {icon: 3, title:'提示'}, function(index){
				  ...
				  $.ajax({
					type: "POST",
				    url: baseURL + "masterdata/testTool/deleteById?id="+grData.ID,
	  ...
	  $("#newOperation").click(function(){
		   var options = {
	    		...
	    		maxmin: true,
	    		shadeClose: true,
	    		title: '<i aria-hidden="true" class="fa fa-plus">&nbsp;</i>新增检具',
	    		area: ["350px", "300px"],
	    		content: "sys/masterdata/test_tool_edit.html",  
	  ...
	  $("#editOperation").click(function(){
		 ...
		 if(gr !== null && gr!==undefined){
			 ...
			 var options = {
				...
				maxmin: true,
				shadeClose: true,
				title: '<i aria-hidden="true" class="fa fa-pencil-square-o">&nbsp;</i>修改检具',
				area: ["350px", "300px"],
				content: 'sys/masterdata/test_tool_edit.html',
				btn: ['<i class="fa fa-check"></i> 确定'],
				success:function(layero, index){
